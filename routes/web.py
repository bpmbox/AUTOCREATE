...
def initialize_gradio_with_error_handling():
    ...
    try:
        ...
        # 正しい順序で再度キューを処理（念のため）
        try:
            # キューが既に初期化されている場合は再制御
            if hasattr(tabbed_interface, '_queue') and tabbed_interface._queue:
                tabbed_interface._queue.max_size = 0
                if hasattr(tabbed_interface._queue, 'event_queue'):
                    tabbed_interface._queue.event_queue = {}
                print("✅ Routes: Gradio interfaces queue re-controlled")
        except Exception as queue_error:
            print(f"⚠️ Routes: Gradio queue re-control warning: {queue_error}")
        ...
        # フォールバックも正しい順序でキュー処理
        try:
            fallback_interface.queue(max_size=1)
            if hasattr(fallback_interface, '_queue'):
                fallback_interface._queue.max_size = 0
        except:
            pass
...
async def gradio_interface():
    ...
    try:
        ...
        # キューを再度無効化（念のため） - Gradio 4.31.5対応
        try:
            ...
            print("✅ Gradio endpoint queue completely disabled")
        except Exception as queue_error:
            print(f"⚠️ Gradio endpoint queue disable warning: {queue_error}")
...
def mount_gradio_interface(app: FastAPI, interface_name: str, mount_path: str):
    ...
    try:
        ...
        try:
            ...
            if target_interface:
                # 個別Gradioマウントを無効化 - app.pyで統一管理
                print(f"⚠️ Individual Gradio mounting DISABLED for {interface_name}")
                print("🔄 All Gradio interfaces managed in unified TabbedInterface")
                return True, f"⚠️ {interface_name} mounting disabled - use unified interface"
            ...
            # フォールバック個別マウントも無効化
            print(f"⚠️ Fallback Gradio mounting DISABLED for {interface_name}")
            return True, f"⚠️ Fallback {interface_name} mounting disabled - use unified interface"
            # fallback_interface = gr.Interface(
