...
def main():
    parser = argparse.ArgumentParser(description="AUTOCREATE WIKI RAG CLI (軽量版)")
    ...
    if args.command == 'build':
        ...
        # ナレッジベース構築（必要に応じて）
        if not rag_system.built:
            print("📚 ナレッジベースを構築中...")
            if not rag_system.build_knowledge_base():
                print("❌ ナレッジベース構築に失敗しました")
                sys.exit(1)
        ...
        print("📊 WIKI RAG システム（軽量版）統計")
        print("-" * 40)
        print(f"総ドキュメント数: {stats['total_documents']}")
        print(f"ベクトル特徴数: {stats['vectorizer_features']}")
        print("WIKIパス:")
