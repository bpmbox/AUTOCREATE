...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function initDebugInfo() {
        ...
        // APIキー状態 - background.jsから直接取得
        try {
            const configResponse = await chrome.runtime.sendMessage({ type: 'GET_SUPABASE_CONFIG' });
            if (configResponse && configResponse.success) {
                if (configResponse.config.keySet) {
                    apiKeyStatus.textContent = `APIキー: 設定済み (${configResponse.config.keyPreview})`;
                } else {
                    apiKeyStatus.textContent = 'APIキー: 未設定';
                }
            } else {
                // フォールバック表示
                apiKeyStatus.textContent = 'APIキー: 確認中...';
            }
        } catch (error) {
            console.error('❌ Supabase設定取得エラー:', error);
            apiKeyStatus.textContent = 'APIキー: エラー';
        }
        ...
        if (lastError.lastError) {
            lastRequestInfo.textContent = `最終エラー: ${lastError.lastError.message.substring(0, 50)}`;
