...
この質問に対して、上記フローに従って完全自動開発を実行してください。
"""
                    ...
                    if result.data:
                        for message in result.data:
                            ...
                            if message_id not in processed_ids:
                                question = message.get('messages', '')
                                
                                if question.strip():
                                    print(f"\n🚀 新着メッセージ検出! ID: {message_id}")
                                    print(f"📝 質問: {question[:100]}...")
                                    
                                    # 動的Mermaid図付きプロンプトを生成
                                    enhanced_prompt = self.get_enhanced_prompt(question)
                                    
                                    # 自動実行
                                    if self.auto_send_to_vscode_chat(enhanced_prompt):
                                    ...
                                    print(f"📝 空メッセージをスキップ: ID {message_id}")
                                    processed_ids.add(message_id)
                                    last_id = max(last_id, message_id)
    ...
    def auto_send_to_vscode_chat(self, message):
        """VS Code Chatに自動でメッセージを送信"""
        try:
            # クリップボードにメッセージをコピー
