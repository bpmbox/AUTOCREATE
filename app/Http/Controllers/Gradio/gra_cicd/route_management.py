...
def get_route_comparison() -> str:
    ...
    try:
        ...
        if total_all > 0:
            ...
            output.append(f"\n🎯 Active Ratio: {active_ratio:.1f}% ({active_routes.get('total_routes', 0)}/{all_summary.get('total_routes', 0)})")
...
def test_gradio_connection() -> str:
    ...
    try:
        cicd = CICDInterface()
        connection_result = cicd.test_gradio_connection()
        
        output = []
        output.append("🔌 Gradio Connection Test")
        output.append("=" * 30)
        output.append(f"Base URL: {cicd.base_url}")
        output.append(f"Status: {'✅ Connected' if connection_result.get('status') == 'success' else '❌ Failed'}")
        
        if connection_result.get('status') == 'success':
            output.append(f"Response Time: {connection_result.get('response_time', 'N/A')}ms")
            output.append(f"API Version: {connection_result.get('api_version', 'N/A')}")
        else:
            output.append(f"Error: {connection_result.get('error', 'Unknown error')}")
        
        return "\n".join(output)
        
    except Exception as e:
        return f"❌ Gradio connection test failed: {str(e)}"

def test_gradio_functions() -> str:
    """Gradio機能テスト"""
    try:
        cicd = CICDInterface()
        functions_result = cicd.test_gradio_functions()
        
        output = []
        output.append("⚙️ Gradio Functions Test")
        output.append("=" * 30)
        
        if functions_result.get('status') == 'success':
            output.append(f"✅ Functions working: {functions_result.get('working_functions', 0)}")
            output.append(f"❌ Functions failed: {functions_result.get('failed_functions', 0)}")
            
            # 詳細表示
            if functions_result.get('details'):
                output.append("\nDetails:")
                for detail in functions_result['details']:
                    status = "✅" if detail.get('status') == 'success' else "❌"
                    output.append(f"  {status} {detail.get('function', 'Unknown')}: {detail.get('message', '')}")
        else:
            output.append(f"❌ Test failed: {functions_result.get('error', 'Unknown error')}")
